import { NpmModuleMaintainerEntity } from "../entity/npmModuleMaintainer.entity";



class ModuleMaintainer {
    async listModuleNamesByUser(user: string) {
        const rows = await NpmModuleMaintainerEntity.findAll({
            attributes: ['name'],
            where: {
                user: user
            }
    ...
    async addMaintainer(name: string, user: string) {
        let row = await NpmModuleMaintainerEntity.findOne({
            where: {
                user: user,
                name: name
            }
        ...
        if (!row) {
            row = await NpmModuleMaintainerEntity.build({
                user: user,
                name: name
            }).save();
        }
    ...
    async removeMaintainers(name: string, users: Array<string> | string) {
        ...
        await NpmModuleMaintainerEntity.destroy({
            where: {
                name: name,
                user: users,
            }
