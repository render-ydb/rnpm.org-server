...
export class libUtil {
    ...
    getCDNKey(name: string, fileName: string) {
    ...
    getUnpublishFileKey(name: string) {
        return `/${name}/sync/unpublish/unpublish-package.json`;
    };

    getPackageFileCDNKey(name: string, version: string) {
        return `/${name}/sync/packages/package-${version}.json`;
    };

    getDistTagCDNKey(name: string, tag: string) {
        return `/${name}/sync/tags/tag-${tag}.json`;
    };

    getSyncTagDir(name: string) {
        return `${name}/sync/tags/`;
    };

    getSyncPackageDir(name: string) {
        return `${name}/sync/packages/`;
    };

    getTagNameFromFileName(fileName: string) {
        const res = fileName.match(TAG_NAME_REG);
        return res && res[1];
    };

    isBackupTagFile(fileName: string) {
    ...
    isBackupPkgFile(fileName: string) {
    ...
    isLocalModule(mods: Array<Record<string, any>>) {
