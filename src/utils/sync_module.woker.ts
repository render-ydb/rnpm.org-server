import EventEmitter = require("events");
import { Json } from "../interface";
import config = require("../appConfig");
import utility = require("utility");
import util = require("util");
import { ModuleLogService } from "../service/module.log.service";
import urllib from "../common/urllib";
import { NpmService } from "../service/npm.service";
import User = require("../models/user");
import {
    isPrivateScopedPackage,
    getSyncPackageDir,
    isBackupPkgFile,
    getSyncTagDir,
    isBackupTagFile,
    getVersionFromFileName,
    getTagNameFromFileName,
    getUnpublishFileKey,
    getTarballFilepath,
    isLocalModule,
    getCDNKey,
    getPackageFileCDNKey,
    getDistTagCDNKey
} from "../lib";
import sleep = require("./sleep");
import mzFs = require('mz/fs');
import fs = require("fs");
import { PackageService } from "../service/package.service";
import { DownloadTotalService } from "../service/download_total_service";
import crypto = require("crypto");
import path = require("path");
import os = require("os");
import thunkify = require('thunkify-wrap');
const urlparse = require('url').parse;


const MAX_LEN = 10 * 1024;
const logService = new ModuleLogService();
const npmSerivce = new NpmService();
const packageService = new PackageService();
const downloadTotalService = new DownloadTotalService();

const USER_AGENT = 'sync.cnpmjs.org/' + config.version +
    ' hostname/' + os.hostname() +
    ' syncModel/' + config.syncModel +
    ' syncInterval/' + config.syncInterval +
    ' syncConcurrency/' + config.syncConcurrency +
    ' ' + urllib.USER_AGENT;


const _listStarUsers = async (modName: string) => {
    var users = await packageService.listStarUserNames(modName);
    var userMap = {};
    users.forEach(function (user) {
        userMap[user] = true;
    });
    return userMap;
}

const _saveNpmUser = async (username: string) => {
    var user = await npmSerivce.getUser(username);
    if (!user) {
        var existsUser = await User.findByName(username);
        if (existsUser && existsUser.npmUser) {
            // delete it
            await User.model.destroy({
                where: {
                    name: username,
                }
            });
            return { exists: true, deleted: true, isNpmUser: true };
        }
        return { exists: false };
    }
    await User.saveNpmUser(user);
    return user;
}

const _saveMaintainer = async (modName: string, username: string, action: string) => {
    if (action === 'add') {
        await packageService.addPublicModuleMaintainer(modName, username);
    } else if (action === 'remove') {
        await packageService.removePublicModuleMaintainer(modName, username);
    }
}

class SyncModuleWorker extends EventEmitter {
    _logId: number;
    _log: string;
    _loging: boolean;
    _isEnd: boolean;
    type: string;
    names: Array<string>;
    startName: string;
    syncPrivatePackage: any;
    username: string;
    concurrency: number;
    _publish: boolean;
    syncUpstreamFirst: any;
    syncFromBackupFile: any;
    syncingNames: Json;
    nameMap: Json;
    noDep: boolean;
    syncDevDependencies: boolean;
    successes: Array<any>;
    fails: Array<any>;
    updates: Array<any>;
    _finished: boolean;
    constructor(options) {
        super();
        EventEmitter.call(this);
        this._logId = options.logId;
        this._log = '';
        this._loging = false;
        this._isEnd = false;
        if (!Array.isArray(options.name)) {
            options.name = [options.name];
        }
        this.type = options.type || 'package';
        this.names = options.name;
        this.startName = this.names[0];

        this.syncPrivatePackage = options.syncPrivatePackage;

        this.username = options.username;
        this.concurrency = options.concurrency || 1;
        this._publish = options.publish === true; // _publish_on_cnpm
        this.syncUpstreamFirst = options.syncUpstreamFirst;
        this.syncFromBackupFile = options.syncFromBackupFile;

        this.syncingNames = {};
        this.nameMap = {};
        this.names.forEach((name) => {
            this.nameMap[name] = true;
        });
        this.noDep = options.noDep === true; // do not sync dependences
        this.syncDevDependencies = config.syncDevDependencies;

        this.successes = [];
        this.fails = [];
        this.updates = [];

        this._finished = false;
    }

    finish() {
        if (this._finished || Object.keys(this.syncingNames).length > 0) {
            return;
        }
        this._finished = true;
        // @ts-ignore
        this.log('[done] Sync %s %s finished, %d success, %d fail\nSuccess: [ %s ]\nFail: [ %s ]',
            this.startName,
            this.type,
            this.successes.length,
            this.fails.length,
            this.successes.join(', '),
            this.fails.join(', '));
    }

    log() {
        var str = '[' + utility.YYYYMMDDHHmmss(new Date()) + '] ' + util.format.apply(util, arguments);
        var logId = this._logId;
        if (logId) {
            if (this._log) {
                this._log += '\n';
            }
            this._log += str;
            if (this._log.length >= MAX_LEN) {
                this._saveLog();
            }
        }
    }

    async _saveLog() {
        if (this._loging) {
            return;
        }

        this._loging = true;
        var logstr = this._log;
        this._log = '';
        if (!logstr) {
            this._loging = false;
            return;
        }
        try {
            await logService.append(this._logId, logstr);
            this._loging = false;
            if (this._log) {
                this._saveLog()
            }
        } catch (error) {
            this._loging = false;
            // ignore the unsave log
            if (this._isEnd) {
                console.error('[SyncModuleWorker] skip to save %s logstr: %s', this._logId, logstr);
            }
        }
    }

    async start() {
        // @ts-ignore
        await this.log('user: %s, sync %s worker start, %d concurrency, nodeps: %s, publish: %s, syncUpstreamFirst: %s',
            this.username, this.names[0], this.concurrency, this.noDep, this._publish, this.syncUpstreamFirst);

        try {
            // sync upstream
            if (this.syncUpstreamFirst) {
                try {
                    await this.syncUpstream(this.startName);
                    this._saveLog();
                } catch (err) {
                    console.error(err);
                }
            }

            if (this.type === 'user') {
                await this.syncUser();
                this._saveLog();
                this._isEnd = true;
                return;
            }

            for (var i = 0; i < this.concurrency; i++) {
                // arr.push();
                await this.next(i)
            }
            this._saveLog();
            await this._saveBackupFiles();
        } catch (error) {
            console.error(error);
            this._saveLog();
        }

    }

    async _saveDistTagBackup(pkgName, tag, version) {
        const cdnKey = getDistTagCDNKey(pkgName, tag);
        const filePath = getTarballFilepath(pkgName, '', `tag-${tag}.json`);
        // @ts-ignore
        this.log('[%s:%s] start backup dist-tag.json', pkgName, tag);
        let oldVersion;
        try {
            await config.nfs.download(cdnKey, filePath);
            oldVersion = await mzFs.readFile(filePath, 'utf8');
            fs.unlink(filePath, utility.noop);
        } catch (_) {
            // ...
        }
        // @ts-ignore
        this.log('[%s:%s] backup dist tag is %j current dist tag is %j', pkgName, tag, oldVersion, version);
        if (oldVersion === version) {
            // @ts-ignore
            this.log('[%s:%s] tag equal skip sync', pkgName, tag);
            return;
        }
        // @ts-ignore
        this.log('[%s:%s] tag not equal start sync', pkgName, tag);
        const file = version;

        await mzFs.writeFile(filePath, file);

        let shasum: any = crypto.createHash('sha1');
        shasum.update(file);
        shasum = shasum.digest('hex');

        await config.nfs.upload(filePath, {
            key: cdnKey,
            size: file.length,
            shasum: shasum,
        });
        // @ts-ignore
        this.log('[%s:%s] backup dist tag success', pkgName, tag);
    }

    async _clearDeletedDistTags(pkgName, tagNames) {
        const syncDir = getSyncTagDir(pkgName);
        const backupDistTagFiless = await config.nfs.list(syncDir);
        const currentTagNames = new Set(tagNames);
        const shouldDelTags = backupDistTagFiless.filter(tagFileName => {
            const tagName = getTagNameFromFileName(tagFileName);
            return (
                // File is an dist-tag file
                tagName
                // tag is deleted
                && !currentTagNames.has(tagName)
            );
        });
        // @ts-ignore
        this.log('[%s] current tags %j backup tags %j should delete tags %j', pkgName, tagNames, backupDistTagFiless, shouldDelTags);
        await Promise.all(shouldDelTags.map(async (tagFileName) => {
            const filePath = path.join(syncDir, tagFileName);
            // @ts-ignore
            this.log('[%s] delete tags %s', pkgName, filePath);
            await config.nfs.remove(filePath);
        }));
        // @ts-ignore
        this.log('[%s] delete tags success', pkgName);
    }

    async _saveBackupFile(pkgName: string) {
        const [mods, tags] = [
            await packageService.listModulesByName(pkgName, ['version']),
            await packageService.listModuleTags(pkgName),
        ];
        await Promise.all(mods.map(async (mod) => {
            await this._savePackageJsonBackup(pkgName, mod.version);
        }));
        await Promise.all(tags.map(async (tag) => {
            await this._saveDistTagBackup(pkgName, tag.tag, tag.version);
        }));
        await this._clearDeletedDistTags(pkgName, tags.map(t => t.tag));
    }

    async _saveBackupFiles() {
        if (!config.syncBackupFiles) {
            return;
        }
        const pkgNames = Object.keys(this.nameMap);
        await Promise.all(pkgNames.map(async (pkgName) => {
            await this._saveBackupFile(pkgName);
        }));
    }

    async syncUpstream(name: string) {
        var sourceNpmRegistry = config.sourceNpmRegistry;
        if (config.enableWebDataRemoteRegistry) {
            sourceNpmRegistry = config.webDataRemoteRegistry;
        }
        if (sourceNpmRegistry.indexOf('registry.npmjs.org') >= 0 ||
            sourceNpmRegistry.indexOf('registry.npmjs.com') >= 0 ||
            sourceNpmRegistry.indexOf('replicate.npmjs.com') >= 0) {
            // @ts-ignore
            this.log('----------------- upstream is npm registry: %s, ignore it -------------------',
                sourceNpmRegistry);
            return;
        }
        var syncname = name;
        if (this.type === 'user') {
            syncname = this.type + ':' + syncname;
        }
        var url = sourceNpmRegistry + '/' + syncname + '/sync?sync_upstream=true';
        if (this.noDep) {
            url += '&nodeps=true';
        }
        var r = await urllib.request(url, {
            method: 'put',
            timeout: 20000,
            headers: {
                'content-length': 0
            },
            dataType: 'json',
            gzip: true,
        });

        if (r.status !== 201 || !r.data.ok) {
            // @ts-ignore
            return this.log('sync upstream %s error, status: %s, response: %j',
                url, r.status, r.data);
        }

        var logURL = sourceNpmRegistry + '/' + name + '/sync/log/' + r.data.logId;
        var offset = 0;
        // @ts-ignore
        this.log('----------------- Syncing upstream %s -------------------', logURL);
        var count = 0;
        while (true) {
            count++;
            var synclogURL = logURL + '?offset=' + offset;
            var rs = await urllib.request(synclogURL, {
                timeout: 20000,
                dataType: 'json',
                gzip: true,
            });

            if (rs.status !== 200 || !rs.data.ok) {
                // @ts-ignore
                this.log('sync upstream %s error, status: %s, response: %j',
                    synclogURL, rs.status, rs.data);
                break;
            }

            var data = rs.data;
            if (data.log) {
                data.log = data.log.replace('[done] Sync', '[Upstream done] Sync');
                // @ts-ignore
                this.log(data.log);
            }

            if (data.syncDone) {
                break;
            }

            if (count >= 30) {
                // @ts-ignore
                this.log('sync upstream %s fail, give up', logURL);
                break;
            }

            if (data.log) {
                offset += data.log.split('\n').length;
            }

            await new Promise(resolve => { setTimeout(resolve, 2000); })
        }
        // @ts-ignore
        this.log('----------------- Synced upstream %s -------------------', logURL);
    }


    async pushSuccess(name: string) {
        this.successes.push(name);
        this.emit('success', name);
    }

    async pushFail(name) {
        this.fails.push(name);
        this.emit('fail', name);
    }
    async syncUser() {
        for (var i = 0; i < this.names.length; i++) {
            var username = this.names[i];
            try {
                var user = await this._saveNpmUser(username);
                this.pushSuccess(username);
                // @ts-ignore
                this.log('[c#%s] [%s] sync success: %j', 0, username, user);
            } catch (err) {
                this.pushFail(username);
                // @ts-ignore
                this.log('[c#%s] [error] [%s] sync error: %s', 0, username, err.stack);
            }
        }
        this.finish();
    }
    async _saveNpmUser(username) {
        var user = await npmSerivce.getUser(username);
        if (!user) {
            var existsUser = await User.findByName(username);
            if (existsUser && existsUser.npmUser) {
                // delete it
                await User.model.destroy({
                    where: {
                        name: username,
                    }
                });
                return { exists: true, deleted: true, isNpmUser: true };
            }
            return { exists: false };
        }
        await User.saveNpmUser(user);
        return user;
    }

    getPrivatePackageDefineRegistry(name: string) {
        if (typeof name !== 'string') return false
        return this.syncPrivatePackage && this.syncPrivatePackage[name.split('/')[0]]
    }


    async next(concurrencyId: number) {
        var name = this.names.shift();
        if (!name) {
            return setImmediate(this.finish.bind(this));
        }

        const defineRegistry = this.getPrivatePackageDefineRegistry(name)

        if (!defineRegistry && config.syncModel === 'none') {
            // @ts-ignore
            this.log('[c#%d] [%s] syncModel is none, ignore',
                concurrencyId, name);
            return this.finish();
        }

        // try to sync from official replicate when no defineRegistry or source npm registry is not cnpmjs.org
        let registry
        if (defineRegistry) {
            registry = defineRegistry
        } else {
            registry = config.sourceNpmRegistryIsCNpm ? config.sourceNpmRegistry : config.officialNpmReplicate;
        }

        await this.syncByName(concurrencyId, name, registry);
    }

    async _doneOne(concurrencyId: number, name: string, success: boolean) {
        // clean cache
        // if (cache) {
        //   const cacheKey = `list-${name}-v1`;
        //   cache.del(cacheKey).catch(err => {
        //     logger.error(err);
        //   });
        // }
        // @ts-ignore
        this.log('----------------- Synced %s %s -------------------',
            name, success ? 'success' : 'fail');
        if (success) {
            this.pushSuccess(name);
        } else {
            this.pushFail(name);
        }
        delete this.syncingNames[name];

        // relase the stack: https://github.com/cnpm/cnpmjs.org/issues/328
        process.nextTick(() => {
            this.next(concurrencyId);
        })
    }

    async isLocalModule(mods) {
        var res = isLocalModule(mods)
        if (!this.syncPrivatePackage) return res
        if (!mods[0] || !mods[0].package || !mods[0].package.name) return res

        if (this.getPrivatePackageDefineRegistry(mods[0].package.name)) return false

        return res
    }

    async _saveUnpublishFile(pkgName: string, pkg) {
        const cdnKey = getUnpublishFileKey(pkgName);
        const filePath = getTarballFilepath(pkgName, '', 'unpublish-package.json');
        // @ts-ignore
        this.log('[%s] start save unpublish-package.json', pkgName);
        const file = JSON.stringify(pkg);
        await mzFs.writeFile(filePath, file);

        let shasum: crypto.Hash | string = crypto.createHash('sha1');
        shasum.update(file);
        shasum = shasum.digest('hex');
        await config.nfs.upload(filePath, {
            key: cdnKey,
            size: file.length,
            shasum: shasum,
        });
        // @ts-ignore
        this.log('[%s:%s] save unpublish package.json backup success', pkgName);
    }

    async _unpublished(name: string, unpublishedInfo) {
        var mods = await packageService.listModulesByName(name);
        // @ts-ignore
        this.log('  [%s] start unpublished %d versions from local cnpm registry',
            name, mods.length);
        if (this.isLocalModule(mods)) {
            // publish on cnpm, dont sync this version package
            // @ts-ignore
            this.log('  [%s] publish on local cnpm registry, don\'t sync', name);
            return [];
        }
        if (!config.syncDeletedVersions) {
            const downloadCount = await downloadTotalService.getTotalByName(name);
            if (downloadCount >= 10000) {
                // @ts-ignore
                this.log('  [%s] total downloads %s >= 10000 and `config.syncDeletedVersions=false`, don\'t sync unpublished info', name, downloadCount);
                return [];
            }
            // @ts-ignore
            this.log('  [%s] total downloads %s < 10000 and `config.syncDeletedVersions=false`, still need to sync unpublished info', name, downloadCount);
        }

        var r = await packageService.saveUnpublishedModule(name, unpublishedInfo);
        // @ts-ignore
        this.log('    [%s] save unpublished info: %j to row#%s',
            name, unpublishedInfo, r.id);
        if (mods.length) {
            await packageService.removeModulesByName(name);
            await packageService.removeModuleTags(name);
            var keys = [];
            for (var i = 0; i < mods.length; i++) {
                var row = mods[i];
                // @ts-ignore
                var dist = row.package.dist;
                var key = dist.key;
                if (!key) {
                    key = urlparse(dist.tarball).pathname;
                }
                key && keys.push(key);
            }

            if (keys.length > 0) {
                try {
                    await Promise.all(keys.map(async (key) => {
                        return await config.nfs.remove(key);
                    }));
                } catch (err) {
                    // ignore error here
                    // @ts-ignore
                    this.log('    [%s] delete nfs files: %j error: %s: %s',
                        name, keys, err.name, err.message);
                }
            }
            // @ts-ignore
            this.log('    [%s] delete nfs files: %j success', name, keys);
        }

        if (config.syncBackupFiles) {
            await this._saveUnpublishFile(name, unpublishedInfo);
        }
    }

    async _sync(name: string, pkg) {
        // var hasModules = false;
        var result = [
            await packageService.listModulesByName(name),
            await packageService.listModuleTags(name),
            await _listStarUsers(name),
            await packageService.listPublicModuleMaintainers(name),
            await packageService.listModuleAbbreviatedsByName(name),
        ];
        var moduleRows: any = result[0];
        var tagRows: any = result[1];
        var existsStarUsers = result[2];
        var existsNpmMaintainers: any = result[3];
        var existsModuleAbbreviateds: any = result[4];

        if (this.isLocalModule(moduleRows)) {
            // publish on cnpm, dont sync this version package
            // @ts-ignore
            this.log('  [%s] publish on local cnpm registry, don\'t sync', name);
            return [];
        }

        var missingModuleAbbreviateds = [];
        var existsModuleAbbreviatedsMap = {};
        existsModuleAbbreviateds
        for (var item of existsModuleAbbreviateds) {
            existsModuleAbbreviatedsMap[item.version] = item;
        }

        // hasModules = moduleRows.length > 0;
        // localPackage
        var map: any = {};
        var localVersionNames = [];
        for (var i = 0; i < moduleRows.length; i++) {
            var r = moduleRows[i];
            if (!r.package || !r.package.dist) {
                // package json parse error
                continue;
            }
            if (!map.latest) {
                map.latest = r;
            }
            map[r.version] = r;
            localVersionNames.push(r.version);
        }

        var latestVersionPackageReadme = {
            version: pkg['dist-tags'].latest,
            readme: pkg.readme,
        };

        var tags = {};
        for (var i = 0; i < tagRows.length; i++) {
            var r = tagRows[i];
            if (!r.module_id) {
                // no module_id, need to sync tags
                continue;
            }
            tags[r.tag] = r.version;
        }

        // get package AbbreviatedMetadata
        var remoteAbbreviatedMetadatas = {};
        // store remote abbreviated versions
        var remoteAbbreviatedVersionsMap = {};
        if (config.enableAbbreviatedMetadata) {
            // use ?cache=0 tell registry dont use cache result
            var packageUrl = '/' + name.replace('/', '%2f') + '?cache=0&sync_timestamp=' + Date.now();
            let result: any = await npmSerivce.request(packageUrl, {
                dataType: 'text',
                registry: config.sourceNpmRegistry,
                headers: {
                    Accept: 'application/vnd.npm.install-v1+json',
                },
            });
            if (result.status === 200) {
                var data;
                try {
                    data = JSON.parse(result.data);
                } catch (err) {
                    // @ts-ignore
                    this.log('  [%s] get abbreviated meta error: %s, headers: %j, %j',
                        name, err, result.headers, result.data);
                }
                if (data) {
                    remoteAbbreviatedVersionsMap = data && data.versions || {};
                    for (var version in remoteAbbreviatedVersionsMap) {
                        const item = remoteAbbreviatedVersionsMap[version];
                        if (!item) {
                            continue;
                        }
                        let hasMetaData = false;
                        const metaData: any = {};
                        // _hasShrinkwrap maybe undefined, dont change it
                        if (typeof item._hasShrinkwrap === 'boolean') {
                            hasMetaData = true;
                            metaData._hasShrinkwrap = item._hasShrinkwrap;
                        }

                        // https://github.com/cnpm/cnpmjs.org/issues/1667
                        const metaDataKeys = [
                            'peerDependenciesMeta', 'os', 'cpu', 'libc', 'workspaces', 'hasInstallScript',
                        ];
                        for (const key of metaDataKeys) {
                            if (key in item) {
                                hasMetaData = true;
                                metaData[key] = item[key];
                            }
                        }
                        if (hasMetaData) {
                            remoteAbbreviatedMetadatas[version] = metaData;
                        }
                    }
                }
            }
        }

        // any properties changed versions
        var changedVersions = {};
        var missingVersions = [];
        var missingTags = [];
        var missingDescriptions = [];
        var missingReadmes = [];
        var missingStarUsers = [];
        var npmUsernames = {};
        var missingDeprecateds = [];
        // [[user, 'add or remove'], ...]
        var diffNpmMaintainers = [];

        // [
        //   { name, version, _hasShrinkwrap(boolean), peerDependenciesMeta(object), os(array), cpu(array), workspaces(array) }
        // ]
        var missingAbbreviatedMetadatas = [];
        // [
        //   { name, version, deprecated },
        // ]
        var missingDeprecatedsOnExistsModuleAbbreviated = [];

        // find out new maintainers
        var pkgMaintainers = pkg.maintainers || [];
        if (Array.isArray(pkgMaintainers)) {
            var existsMap = {};
            var originalMap = {};
            for (var i = 0; i < existsNpmMaintainers.length; i++) {
                var user = existsNpmMaintainers[i];
                existsMap[user] = true;
            }
            for (var i = 0; i < pkgMaintainers.length; i++) {
                var item = pkgMaintainers[i];
                originalMap[item.name] = item;
                npmUsernames[item.name.toLowerCase()] = 1;
            }

            // find add users
            for (var i = 0; i < pkgMaintainers.length; i++) {
                var item = pkgMaintainers[i];
                if (!existsMap[item.name]) {
                    diffNpmMaintainers.push([item.name, 'add']);
                }
            }

            // find remove users
            for (var i = 0; i < existsNpmMaintainers.length; i++) {
                var user = existsNpmMaintainers[i];
                if (!originalMap[user]) {
                    diffNpmMaintainers.push([user, 'remove']);
                }
            }
        }

        // find out all user names
        for (var v in pkg.versions) {
            var p = pkg.versions[v];
            var maintainers = p.maintainers || [];
            if (!Array.isArray(maintainers)) {
                // http://r.cnpmjs.org/jasmine-node
                // TODO: "maintainers": "Martin H膫陇ger <martin.haeger@gmail.com>",
                maintainers = [maintainers];
            }
            for (var i = 0; i < maintainers.length; i++) {
                var m = maintainers[i];
                if (m.name) {
                    npmUsernames[m.name.toLowerCase()] = 1;
                }
            }
        }

        // get the missing star users
        var starUsers = pkg.users || {};
        for (var k in starUsers) {
            if (!existsStarUsers[k]) {
                missingStarUsers.push(k);
            }
            npmUsernames[k.toLowerCase()] = 1;
        }
        // @ts-ignore
        this.log('  [%s] found %d missing star users', name, missingStarUsers.length);

        var times = pkg.time || {};
        pkg.versions = pkg.versions || {};
        var remoteVersionNames = Object.keys(pkg.versions);
        var remoteVersionNameMap = {};

        // find out missing versions
        for (var i = 0; i < remoteVersionNames.length; i++) {
            var v = remoteVersionNames[i];
            remoteVersionNameMap[v] = v;
            var exists = map[v] || {};
            let version: any = pkg.versions[v];
            if (!version || !version.dist || !version.dist.tarball) {
                continue;
            }
            // remove readme
            if (config.enableAbbreviatedMetadata) {
                version.readme = undefined;
            } else {
                // patch for readme
                if (!version.readme) {
                    version.readme = pkg.readme;
                }
            }

            var publish_time = times[v];
            version.publish_time = publish_time ? Date.parse(publish_time) : null;
            if (!version.maintainers || !version.maintainers[0]) {
                version.maintainers = pkg.maintainers;
            }

            var abbreviatedMetadata = remoteAbbreviatedMetadatas[version.version];

            if (exists.package && exists.package.dist.shasum === version.dist.shasum) {
                var existsModuleAbbreviated = existsModuleAbbreviatedsMap[exists.package.version];
                if (!existsModuleAbbreviated) {
                    missingModuleAbbreviateds.push(exists);
                } else {
                    // sync missing deprecated on existsModuleAbbreviated
                    if (exists.package.deprecated && exists.package.deprecated !== existsModuleAbbreviated.package.deprecated) {
                        // add deprecated
                        missingDeprecatedsOnExistsModuleAbbreviated.push({
                            name,
                            version: exists.package.version,
                            deprecated: exists.package.deprecated,
                        });
                    } else if (existsModuleAbbreviated.package.deprecated && !exists.package.deprecated) {
                        // remove deprecated
                        missingDeprecatedsOnExistsModuleAbbreviated.push({
                            name,
                            version: exists.package.version,
                            deprecated: undefined,
                        });
                    }
                }

                // * shasum make sure equal
                if ((version.publish_time === exists.publish_time) ||
                    (!version.publish_time && exists.publish_time)) {
                    // debug('  [%s] %s publish_time equal: %s, %s',
                    //   name, version.version, version.publish_time, exists.publish_time);
                    // * publish_time make sure equal
                    if (exists.description === null && version.description) {
                        // * make sure description exists
                        missingDescriptions.push({
                            id: exists.id,
                            description: version.description
                        });
                        changedVersions[v] = 1;
                    }

                    if (config.enableAbbreviatedMetadata) {
                        // remove readme
                        if (exists.package.readme) {
                            missingReadmes.push({
                                id: exists.id,
                                readme: undefined,
                            });
                        }
                    } else {
                        if (!exists.package.readme && version.readme) {
                            // * make sure readme exists
                            missingReadmes.push({
                                id: exists.id,
                                readme: version.readme,
                            });
                            changedVersions[v] = 1;
                        }
                    }

                    if (version.deprecated && version.deprecated !== exists.package.deprecated) {
                        // need to sync deprecated field
                        missingDeprecateds.push({
                            id: exists.id,
                            deprecated: version.deprecated,
                        });
                        changedVersions[v] = 1;
                    }
                    if (exists.package.deprecated && !version.deprecated) {
                        // remove deprecated info
                        missingDeprecateds.push({
                            id: exists.id,
                            deprecated: undefined,
                        });
                        changedVersions[v] = 1;
                    }

                    // find missing abbreviatedMetadata
                    if (abbreviatedMetadata) {
                        for (var key in abbreviatedMetadata) {
                            const value = abbreviatedMetadata[key];
                            // boolean: _hasShrinkwrap, hasInstallScript
                            if ((key === '_hasShrinkwrap' || key === 'hasInstallScript') && typeof value === 'boolean') {
                                if (!(key in exists.package) || abbreviatedMetadata[key] !== exists.package[key]) {
                                    missingAbbreviatedMetadatas.push(Object.assign({
                                        id: exists.id,
                                        name: exists.package.name,
                                        version: exists.package.version,
                                    }, abbreviatedMetadata));
                                    break;
                                }
                            } else if (Array.isArray(value) || (typeof value === 'object' && value)) {
                                // array: os, cpu, workspaces, libc
                                // object: peerDependenciesMeta
                                if (existsModuleAbbreviated && !(key in existsModuleAbbreviated.package)) {
                                    missingAbbreviatedMetadatas.push(Object.assign({
                                        id: exists.id,
                                        name: exists.package.name,
                                        version: exists.package.version,
                                    }, abbreviatedMetadata));
                                    break;
                                }
                            }
                        }
                    }

                    continue;
                }
            }

            // set abbreviatedMetadata to version package
            if (abbreviatedMetadata) {
                Object.assign(version, abbreviatedMetadata);
            }
            missingVersions.push(version);
            changedVersions[v] = 1;
        }

        // find out deleted versions
        var deletedVersionNames = [];
        for (var i = 0; i < localVersionNames.length; i++) {
            let v = localVersionNames[i];
            if (!remoteVersionNameMap[v]) {
                deletedVersionNames.push(v);
            }
        }
        // delete local abbreviatedMetadata data too
        for (var item of existsModuleAbbreviateds) {
            if (!remoteVersionNameMap[item.version] && deletedVersionNames.indexOf(item.version) === -1) {
                deletedVersionNames.push(item.version);
            }
        }

        // find out missing tags
        var sourceTags = pkg['dist-tags'] || {};
        for (var t in sourceTags) {
            var sourceTagVersion = sourceTags[t];
            if (sourceTagVersion && tags[t] !== sourceTagVersion) {
                missingTags.push([t, sourceTagVersion]);
            }
        }
        // find out deleted tags
        var deletedTags = [];
        for (var t in tags) {
            if (!sourceTags[t]) {
                // not in remote tags, delete it from local registry
                deletedTags.push(t);
            }
        }

        if (missingVersions.length === 0) {
            // @ts-ignore
            this.log('  [%s] all versions are exists', name);
        } else {
            missingVersions.sort(function (a, b) {
                return a.publish_time - b.publish_time;
            });
            // @ts-ignore
            this.log('  [%s] %d versions need to sync', name, missingVersions.length);
        }

        var syncedVersionNames = [];
        var syncIndex = 0;

        // sync missing versions
        while (missingVersions.length) {
            var index = syncIndex++;
            var syncModule = missingVersions.shift();
            if (!syncModule.dist.tarball) {
                continue;
            }
            // retry 3 times
            var tries = 3;
            while (true) {
                try {
                    await this._syncOneVersion(index, syncModule, remoteAbbreviatedVersionsMap[syncModule.version]);
                    syncedVersionNames.push(syncModule.version);
                    break;
                } catch (err) {
                    var delay = Date.now() - syncModule.publish_time;
                    // @ts-ignore
                    this.log('    [%s:%d] tries: %d, delay: %s ms, sync error, version: %s, %s: %s',
                        syncModule.name, index, tries, delay, syncModule.version, err.name, err.stack);
                    var maxDelay = 3600000;
                    if (tries-- > 0 && delay < maxDelay) {
                        // @ts-ignore
                        this.log('    [%s:%d] retry after 30s', syncModule.name, index);
                        await sleep(30000);
                    } else {
                        break;
                    }
                }
            }
        }

        if (deletedVersionNames.length === 0) {
            // @ts-ignore
            this.log('  [%s] no versions need to deleted', name);
        } else {
            if (config.syncDeletedVersions) {
                // @ts-ignore
                this.log('  [%s] %d versions: %j need to deleted, because config.syncDeletedVersions=true',
                    name, deletedVersionNames.length, deletedVersionNames);
                try {
                    await packageService.removeModulesByNameAndVersions(name, deletedVersionNames);
                } catch (err) {
                    // @ts-ignore
                    this.log('    [%s] delete error, %s: %s', name, err.name, err.message);
                }
            } else {
                const downloadCount = await downloadTotalService.getTotalByName(name);
                if (downloadCount >= 10000) {
                    // find deleted in 24 hours versions
                    var oneDay = 3600000 * 24;
                    var now = Date.now();
                    var deletedIn24HoursVersions = [];
                    var oldVersions = [];
                    for (var i = 0; i < deletedVersionNames.length; i++) {
                        let v: any = deletedVersionNames[i];
                        var exists = map[v];
                        if (exists && now - exists.publish_time < oneDay) {
                            deletedIn24HoursVersions.push(v);
                        } else {
                            oldVersions.push(v);
                        }
                    }
                    if (deletedIn24HoursVersions.length > 0) {
                        // @ts-ignore
                        this.log('  [%s] %d versions: %j need to deleted, because they are deleted in 24 hours',
                            name, deletedIn24HoursVersions.length, deletedIn24HoursVersions);
                        try {
                            await packageService.removeModulesByNameAndVersions(name, deletedIn24HoursVersions);
                        } catch (err) {
                            // @ts-ignore
                            this.log('    [%s] delete error, %s: %s', name, err.name, err.message);
                        }
                    }
                    // @ts-ignore
                    this.log('  [%s] %d versions: %j no need to delete, because `config.syncDeletedVersions=false`',
                        name, oldVersions.length, oldVersions);
                } else {
                    // @ts-ignore
                    this.log('  [%s] %d versions: %j need to deleted, because downloads %s < 10000',
                        name, deletedVersionNames.length, deletedVersionNames, downloadCount);
                    try {
                        await packageService.removeModulesByNameAndVersions(name, deletedVersionNames);
                    } catch (err) {
                        // @ts-ignore
                        this.log('    [%s] delete error, %s: %s', name, err.name, err.message);
                    }
                }
            }
        }

        // sync missing descriptions
        const syncDes = async () => {
            if (missingDescriptions.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d descriptions', name, missingDescriptions.length);
            var res = await Promise.all(missingDescriptions.map(async (item) => {
                return await packageService.updateModuleDescription(item.id, item.description);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingDescriptions[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, id: %s, description: %s, error: %s',
                        // @ts-ignore
                        item.id, item.description, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    saved, id: %s, description length: %d',
                        item.id, item.description.length);
                }
            }
        }

        // sync missing tags
        const syncTag = async () => {
            if (deletedTags.length > 0) {
                await packageService.removeModuleTagsByNames(name, deletedTags);
                // @ts-ignore
                this.log('  [%s] deleted %d tags: %j',
                    name, deletedTags.length, deletedTags);
            }


            if (missingTags.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] adding %d tags', name, missingTags.length);
            // sync tags
            var res = await Promise.all(missingTags.map(async (item) => {
                return await packageService.addModuleTag(name, item[0], item[1]);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingTags[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    add tag %s:%s error, error: %s',
                        // @ts-ignore
                        item.id, item.description, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    added tag %s:%s, module_id: %s',
                        // @ts-ignore
                        item[0], item[1], r.value && r.value.module_id);
                }
            }
        }

        // sycn missing readme
        const syncReadme = async () => {
            if (missingReadmes.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d readmes', name, missingReadmes.length);

            var res = await Promise.all(missingReadmes.map(async (item) => {
                return await packageService.updateModuleReadme(item.id, item.readme);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingReadmes[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, id: %s, error: %s', item.id, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    saved, id: %s', item.id);
                }
            }
        }

        const syncModuleAbbreviateds = async () => {
            if (missingModuleAbbreviateds.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d missing moduleAbbreviateds', name, missingModuleAbbreviateds.length);

            var res = await Promise.all(missingModuleAbbreviateds.map(async (item) => {
                return await packageService.saveModuleAbbreviated(item, remoteAbbreviatedVersionsMap[item.version]);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingModuleAbbreviateds[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save moduleAbbreviateds error, module: %s@%s, error: %s', item.name, item.version, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    saved moduleAbbreviateds, module: %s@%s', item.name, item.version);
                }
            }
        }

        const syncAbbreviatedMetadatas = async () => {
            if (missingAbbreviatedMetadatas.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d abbreviated meta datas', name, missingAbbreviatedMetadatas.length);

            var res: any = await Promise.all(missingAbbreviatedMetadatas.map(async (item) => {
                return await packageService.updateModuleAbbreviatedPackage(item);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingAbbreviatedMetadatas[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, module_abbreviated: %s@%s, error: %s',
                        // @ts-ignore  
                        item.name, item.version, r.error.stack);
                } else {
                    // @ts-ignore
                    this.log('    saved, module_abbreviated: %s@%s, %j', item.name, item.version, item);
                }
            }

            res = await Promise.all(missingAbbreviatedMetadatas.map(async (item) => {
                var fields = {};
                for (var key in item) {
                    if (key === 'id' || key === 'name' || key === 'version') {
                        continue;
                    }
                    fields[key] = item[key];
                }
                return await packageService.updateModulePackageFields(item.id, fields);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingAbbreviatedMetadatas[i];
                var r = res[i];
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, module id: %s, error: %s', item.id, r.error.stack);
                } else {
                    // @ts-ignore
                    this.log('    saved, module id: %s, %j', item.id, item);
                }
            }
        }

        const syncDeprecatedsOnExistsModuleAbbreviated = async () => {
            if (missingDeprecatedsOnExistsModuleAbbreviated.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d module abbreviated deprecated fields',
                name, missingDeprecatedsOnExistsModuleAbbreviated.length);

            var res = await Promise.all(missingDeprecatedsOnExistsModuleAbbreviated.map(async (item) => {
                return await packageService.updateModuleAbbreviatedPackage(item);
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingDeprecatedsOnExistsModuleAbbreviated[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, module abbreviated: %s@%s, error: %s', item.name, item.version, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    saved, module abbreviated: %s@%s, deprecated: %j', item.name, item.version, item.deprecated);
                }
            }
        }

        const syncDeprecateds = async () => {
            if (missingDeprecateds.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d Deprecated fields', name, missingDeprecateds.length);

            var res = await Promise.all(missingDeprecateds.map(async (item) => {
                return await packageService.updateModulePackageFields(item.id, {
                    deprecated: item.deprecated
                });
            }));

            for (var i = 0; i < res.length; i++) {
                var item = missingDeprecateds[i];
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save error, id: %s, error: %s', item.id, r.error.message);
                } else {
                    // @ts-ignore
                    this.log('    saved, id: %s, deprecated: %j', item.id, item.deprecated);
                }
            }
        }

        const syncMissingUsers = async () => {
            var missingUsers = [];
            var names = Object.keys(npmUsernames);
            if (names.length === 0) {
                return;
            }
            var rows = await User.listByNames(names);
            var map = {};
            rows.forEach(function (r) {
                map[r.name] = r;
            });
            names.forEach(function (username) {
                var r = map[username];
                if (!r || !r.json) {
                    if (username[0] !== '"' && username[0] !== "'") {
                        missingUsers.push(username);
                    }
                }
            });

            if (missingUsers.length === 0) {
                // @ts-ignore
                this.log('  [%s] all %d npm users exists', name, names.length);
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d/%d missing npm users: %j',
                name, missingUsers.length, names.length, missingUsers);
            var res = await Promise.all(missingUsers.map(async (username) => {
                return await _saveNpmUser(username);
            }));

            for (var i = 0; i < res.length; i++) {
                var r = res[i];
                if (r.error) {
                    // @ts-ignore
                    this.log('    save npm user error, %s', r.error.message);
                }
            }
        }

        // sync missing star users
        const syncMissingStarUsers = async () => {
            if (missingStarUsers.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] saving %d star users', name, missingStarUsers.length);
            var res = await Promise.all(missingStarUsers.map(async (username) => {
                return await packageService.addStar(name, username);
            }));

            for (var i = 0; i < res.length; i++) {
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    add star user error, %s', r.error.stack);
                }
            }
        }

        // sync diff npm package maintainers
        const syncNpmPackageMaintainers = async () => {
            if (diffNpmMaintainers.length === 0) {
                return;
            }
            // @ts-ignore
            this.log('  [%s] syncing %d diff package maintainers: %j',
                name, diffNpmMaintainers.length, diffNpmMaintainers);
            var res = await Promise.all(diffNpmMaintainers.map(async (item) => {
                return _saveMaintainer(name, item[0], item[1]);
            }));

            for (var i = 0; i < res.length; i++) {
                var r = res[i];
                // @ts-ignore
                if (r.error) {
                    // @ts-ignore
                    this.log('    save package maintainer error, %s', r.error.stack);
                }
            }
        }

        if (latestVersionPackageReadme.version && latestVersionPackageReadme.readme) {
            var existsPackageReadme = await packageService.getPackageReadme(name, true);
            if (!existsPackageReadme ||
                existsPackageReadme.version !== latestVersionPackageReadme.version ||
                existsPackageReadme.readme !== latestVersionPackageReadme.readme) {
                var r: any = await packageService.savePackageReadme(name, latestVersionPackageReadme.readme, latestVersionPackageReadme.version);
                // @ts-ignore
                this.log('    save packageReadme: %s %s %s', r.id, r.name, r.version);
            }
        }

        await syncDes();
        await syncTag();
        await syncReadme();
        await syncDeprecateds();
        await syncMissingStarUsers();
        await syncMissingUsers();
        await syncNpmPackageMaintainers();
        await syncModuleAbbreviateds();
        await syncAbbreviatedMetadatas();
        await syncDeprecatedsOnExistsModuleAbbreviated();

        changedVersions = Object.keys(changedVersions);
        // // hooks
        // const envelope = {
        //     event: 'package:sync',
        //     name: name,
        //     type: 'package',
        //     version: null,
        //     hookOwner: null,
        //     payload: {
        //         changedVersions,
        //     },
        //     change: null,
        // };
        // hook.trigger(envelope);

        return syncedVersionNames;
    }

    async _syncByNameFromBackupFile(concurrencyId: number, name: string, retryCount: number) {

        const validateUnpublish = async (name: string) => {
            const filePath = getTarballFilepath(name, '', `unpublish-package.json`);
            const cdnKey = getUnpublishFileKey(name);
            let unpublishInfo;
            try {
                await config.nfs.download(cdnKey, filePath);
                const packageJSONFile = await mzFs.readFile(filePath, 'utf8');
                unpublishInfo = JSON.parse(packageJSONFile);
            } catch (_) {
                // ...
                return false;
            } finally {
                fs.unlink(filePath, utility.noop);
            }
            // @ts-ignore
            this.log('[c#%s] get unpublish info', concurrencyId, name);
            await this._unpublished(name, unpublishInfo);
            return true;
        }


        const readPackage = async (name, version) => {
            const filePath = getTarballFilepath(name, version, `package-${version}.json`);
            const packageJsonKey = getPackageFileCDNKey(name, version);
            try {
                await config.nfs.download(packageJsonKey, filePath);
                const packageJSONFile = await mzFs.readFile(filePath, 'utf8');
                console.log('file: ', filePath, packageJSONFile);
                const packageJSON = JSON.parse(packageJSONFile);
                return packageJSON;
            } finally {
                fs.unlink(filePath, utility.noop);
            }
        }

        const readDistTag = async (name, tag) => {
            const filePath = getTarballFilepath(name, '', `tag-${tag}.json`);
            const packageJsonKey = getDistTagCDNKey(name, tag);
            try {
                await config.nfs.download(packageJsonKey, filePath);
                const version = await mzFs.readFile(filePath, 'utf8');
                return version;
            } finally {
                fs.unlink(filePath, utility.noop);
            }
        }

        this.syncingNames[name] = true;
        // @ts-ignore
        this.log('----------------- Syncing %s -------------------', name);

        // ignore private scoped package
        if (isPrivateScopedPackage(name)) {
            // @ts-ignore
            this.log('[c#%d] [%s] ignore sync private scoped %j package',
                concurrencyId, name, config.scopes);
            await this._doneOne(concurrencyId, name, true);
            return;
        }

        // validate if unpublish
        const unpublished = await validateUnpublish(name);
        if (unpublished) {
            // @ts-ignore
            this.log('[c#%d] [%s] package is unpublished skip sync',
                concurrencyId, name);
            await this._doneOne(concurrencyId, name, true);
            return;
        }

        let packageJsons;
        let tags;
        try {
            const packageDir = getSyncPackageDir(name);
            const packageDirFiles = await config.nfs.list(packageDir);
            const packageJsonFileNames = packageDirFiles.filter(fileName => isBackupPkgFile(fileName));

            const distTagDir = getSyncTagDir(name);
            const distTagDirFiles = await config.nfs.list(distTagDir);
            const distTagFileNames = distTagDirFiles.filter(fileName => isBackupTagFile(fileName));

            const packageJsonRes = await Promise.all(packageJsonFileNames.map(async (packageJsonFileName) => {
                const version = getVersionFromFileName(packageJsonFileName);
                return await readPackage(name, version);
            }));
            packageJsons = packageJsonRes.map(({ isError, error, value }) => {
                if (isError) {
                    error.message = '[sync] read package.json failed: ' + error.message;
                    throw error;
                }
                return value;
            });

            packageJsons = packageJsons.sort((a, b) => {
                return a.publish_time - b.publish_time;
            });

            const tagRes = await Promise.all(distTagFileNames.map(async (tagFileName) => {
                const tag = getTagNameFromFileName(tagFileName);
                const version = await readDistTag(name, tag);
                return {
                    tag,
                    version,
                };
            }));
            tags = tagRes.map(({ isError, error, value }) => {
                if (isError) {
                    error.message = '[sync] read dist-tag failed: ' + error.message;
                    throw error;
                }
                return value;
            });
        } catch (err) {
            if (retryCount < 3) {
                // @ts-ignore
                this.log('[c#%d] [%s] retry from oss after 3s, err: %s, retryCount: %s',
                    concurrencyId, name, err.stack, retryCount);
                await sleep(3000);
                await this._syncByNameFromBackupFile(concurrencyId, name, retryCount + 1);
                return;
            }
            // @ts-ignore
            this.log('[c#%s] [error] [%s] sync error: %s', concurrencyId, name, err.stack);
            await this._doneOne(concurrencyId, name, false);
            return;
        }

        const firstPkg = packageJsons[0];
        const lastPkg = packageJsons[packageJsons.length - 1];

        const times = packageJsons.reduce((times, packageJson) => {
            times[packageJson.version] = new Date(packageJson.publish_time);
            return times;
        }, {
            modified: new Date(lastPkg.publish_time),
            created: new Date(firstPkg.publish_time),
        });
        const distTags = tags.reduce((distTags, tag) => {
            distTags[tag.tag] = tag.version;
            return distTags;
        }, {});
        const versions = packageJsons.reduce((versions, packageJson) => {
            versions[packageJson.version] = packageJson;
            return versions;
        }, {});

        const pkg = {
            name,
            'dist-tags': distTags,
            versions: versions,
            time: times,

            description: lastPkg.description,
            maintainers: lastPkg.maintainers,
            author: lastPkg.author,
            repository: lastPkg.repository,
            readme: lastPkg.readme,
            readmeFilename: lastPkg.readmeFilename,
            homepage: lastPkg.homepage,
            bugs: lastPkg.bugs,
            license: lastPkg.license,
        };

        let syncVersions;
        try {
            syncVersions = await this._sync(name, pkg);
        } catch (err) {
            // @ts-ignore
            this.log('[c#%s] [error] [%s] sync error: %s', concurrencyId, name, err.stack);
            await this._doneOne(concurrencyId, name, false);
            return;
        }

        // has new version
        if (syncVersions.length > 0) {
            this.updates.push(name);
        }
        // @ts-ignore
        this.log('[c#%d] [%s] synced success, %d versions: %s',
            concurrencyId, name, syncVersions.length, syncVersions.join(', '));
        await this._doneOne(concurrencyId, name, true);

        return syncVersions;



    }

    async syncByName(concurrencyId: number, name: string, registry: string, retryCount?: number) {
        if (this.syncFromBackupFile) {
            await this._syncByNameFromBackupFile(concurrencyId, name, retryCount);
            return;
        }

        retryCount = retryCount || 0;
        this.syncingNames[name] = true;
        var pkg = null;
        var status = 0;
        // @ts-ignore
        this.log('----------------- Syncing %s -------------------', name);

        const isNeedSyncPrivatePackage = this.getPrivatePackageDefineRegistry(name)
        // ignore private scoped package
        if (!isNeedSyncPrivatePackage && isPrivateScopedPackage(name)) {
            // @ts-ignore
            this.log('[c#%d] [%s] ignore sync private scoped %j package',
                concurrencyId, name, config.scopes);
            await this._doneOne(concurrencyId, name, true);
            return;
        }

        let realRegistry = registry;
        // get from npm, don't cache
        const packageUrl = '/' + name.replace('/', '%2f') + '?cache=0&sync_timestamp=' + Date.now();
        try {
            var result = await npmSerivce.request(packageUrl, { registry: registry });
            pkg = result.data;
            status = result.status;
            // read from officialNpmRegistry and use the latest modified package info
            if (registry === config.officialNpmReplicate) {
                try {
                    const officialResult = await npmSerivce.request(packageUrl, { registry: config.officialNpmRegistry });
                    const officialPkg = officialResult.data;
                    const officialStatus = officialResult.status;
                    // @ts-ignore
                    this.log('[c#%d] [%s] official registry(%j, %j), replicate(%j, %j), packageUrl: %s',
                        concurrencyId, name,
                        officialPkg['dist-tags'], officialPkg.time && officialPkg.time.modified,
                        pkg['dist-tags'], pkg.time && pkg.time.modified, packageUrl);
                    if (officialPkg.time) {
                        if (!pkg.time || officialPkg.time.modified > pkg.time.modified) {
                            // @ts-ignore
                            this.log('[c#%d] [%s] use official registry\'s data instead of replicate, modified: %j < %j',
                                concurrencyId, name, pkg.time && pkg.time.modified, officialPkg.time.modified);
                            pkg = officialPkg;
                            status = officialStatus;
                            realRegistry = config.officialNpmRegistry;
                        }
                    }
                } catch (err) {
                    // @ts-ignore
                    this.log('[c#%s] [error] [%s] get package(%s%s) error: %s',
                        concurrencyId, name, config.officialNpmReplicate, packageUrl, err);
                }
            }
        } catch (err) {
            // if 404
            if (!err.res || err.res.statusCode !== 404) {
                var errMessage = err.name + ': ' + err.message;
                // @ts-ignore
                this.log('[c#%s] [error] [%s] get package(%s%s) error: %s, status: %s, retryCount: %s',
                    concurrencyId, name, registry, packageUrl, errMessage, status, retryCount);

                // retry from cnpmRegistry again, max 3 times
                if (registry === config.cnpmRegistry && retryCount < 3) {
                    // @ts-ignore
                    this.log('[c#%d] [%s] retry from %s after 3s, retryCount: %s',
                        concurrencyId, name, registry, retryCount);
                    await sleep(3000);
                    await this.syncByName(concurrencyId, name, registry, retryCount + 1);
                    return;
                }

                // replicate/cnpmRegistry request error, try to request from official registry
                if (registry !== config.officialNpmReplicate && registry !== config.cnpmRegistry) {
                    // sync fail
                    await this._doneOne(concurrencyId, name, false);
                    return;
                }

                // retry from officialNpmRegistry when officialNpmReplicate fail
                // @ts-ignore
                this.log('[c#%d] [%s] retry from %s, retryCount: %s',
                    concurrencyId, name, config.officialNpmRegistry, retryCount);
                try {
                    var result = await npmSerivce.request(packageUrl, { registry: config.officialNpmRegistry });
                    pkg = result.data;
                    status = result.status;
                    realRegistry = config.officialNpmRegistry;
                } catch (err) {
                    var errMessage = err.name + ': ' + err.message;
                    // @ts-ignore
                    this.log('[c#%s] [error] [%s] get package(%s%s) error: %s, status: %s',
                        concurrencyId, name, config.officialNpmRegistry, packageUrl, errMessage, status);
                    // sync fail
                    await this._doneOne(concurrencyId, name, false);
                    return;
                }
            }
        }

        if (status === 404 && pkg && pkg.reason && registry === config.officialNpmReplicate) {
            // unpublished package on replicate.npmjs.com
            // 404 { error: 'not_found', reason: 'deleted' }
            // try to read from registry.npmjs.com and get the whole unpublished info

            // https://replicate.npmjs.com/registry-url
            // {"error":"not_found","reason":"no_db_file"}
            // try to read from registry.npmjs.com and get no_db_file
            try {
                var result = await npmSerivce.request(packageUrl, { registry: config.sourceNpmRegistry });
                pkg = result.data;
                status = result.status;
                realRegistry = config.sourceNpmRegistry;
            } catch (err) {
                // if 404
                if (!err.res || err.res.statusCode !== 404) {
                    var errMessage = err.name + ': ' + err.message;
                    // @ts-ignore
                    this.log('[c#%s] [error] [%s] get package(%s%s) error: %s, status: %s',
                        concurrencyId, name, config.sourceNpmRegistry, packageUrl, errMessage, status);
                    await this._doneOne(concurrencyId, name, false);
                    return;
                }
            }
        }

        var unpublishedInfo = null;
        if (status === 404) {
            // check if it's unpublished
            // ignore too long package name, see https://github.com/cnpm/cnpmjs.org/issues/1066
            if (name.length < 80 && pkg && pkg.time && pkg.time.unpublished && pkg.time.unpublished.time) {
                unpublishedInfo = pkg.time.unpublished;
            } else {
                pkg = null;
            }
        } else {
            // unpublished package status become to 200
            if (name.length < 80 && pkg && pkg.time && pkg.time.unpublished && pkg.time.unpublished.time) {
                unpublishedInfo = pkg.time.unpublished;
            }
        }

        if (!pkg) {
            // @ts-ignore
            this.log('[c#%s] [error] [%s] get package(%s%s) error: package not exists, status: %s',
                concurrencyId, name, realRegistry, packageUrl, status);
            await this._doneOne(concurrencyId, name, true);
            // return empty versions, try again on officialNpmRegistry
            return [];
        }
        // @ts-ignore
        this.log('[c#%d] [%s] package(%s%s) status: %s, dist-tags: %j, time.modified: %s, unpublished: %j, start...',
            concurrencyId, name, realRegistry, packageUrl, status,
            pkg['dist-tags'], pkg.time && pkg.time.modified,
            unpublishedInfo);

        if (unpublishedInfo) {
            try {
                await this._unpublished(name, unpublishedInfo);
            } catch (err) {
                // @ts-ignore
                this.log('[c#%s] [error] [%s] sync error: %s', concurrencyId, name, err.stack);
                await this._doneOne(concurrencyId, name, false);
                return;
            }
            return await this._doneOne(concurrencyId, name, true);
        }

        var versions;
        try {
            versions = await this._sync(name, pkg);
        } catch (err) {
            // @ts-ignore
            this.log('[c#%s] [error] [%s] sync error: %s', concurrencyId, name, err.stack);
            await this._doneOne(concurrencyId, name, false);
            return;
        }

        // has new version
        if (versions.length > 0) {
            this.updates.push(name);
        }
        // @ts-ignore
        this.log('[c#%d] [%s] synced success, %d versions: %s',
            concurrencyId, name, versions.length, versions.join(', '));
        await this._doneOne(concurrencyId, name, true);

        return versions;
    }
    async _syncOneVersion(versionIndex, sourcePackage, remoteAbbreviatedVersion) {
        var delay = Date.now() - sourcePackage.publish_time;
        // logger.syncInfo('[sync_module_worker] delay: %s ms, publish_time: %s, start sync %s@%s',
        //   delay, utility.logDate(new Date(sourcePackage.publish_time)),
        //   sourcePackage.name, sourcePackage.version);
        var username = this.username;
        var downurl = sourcePackage.dist.tarball;
        var urlobj = urlparse(downurl);
        // let cnpmjs.org registry know this request send from sync worker
        if (downurl.indexOf('?') > 0) {
            downurl = `${downurl}&cache=0&sync_timestamp=${Date.now()}`;
        } else {
            downurl = `${downurl}?cache=0&sync_timestamp=${Date.now()}`;
        }
        var filename = path.basename(urlobj.pathname);
        var filepath = getTarballFilepath(sourcePackage.name, sourcePackage.version, filename);
        var ws = fs.createWriteStream(filepath);

        var downloadOptions = {
            writeStream: ws,
            followRedirect: true,
            timeout: 600000, // 10 minutes download
            headers: {
                'user-agent': USER_AGENT,
            },
            gzip: true,
        };
        if (config.syncDownloadOptions) {
            Object.assign(downloadOptions, config.syncDownloadOptions);
        }

        var dependencies = Object.keys(sourcePackage.dependencies || {});
        var devDependencies = [];
        if (this.syncDevDependencies) {
            devDependencies = Object.keys(sourcePackage.devDependencies || {});
        }
        // @ts-ignore
        this.log('    [%s:%d] syncing, delay: %s ms, version: %s, dist: %j, no deps: %s, ' +
            'publish on cnpm: %s, dependencies: %d, devDependencies: %d, syncDevDependencies: %s',
            sourcePackage.name, versionIndex,
            delay,
            sourcePackage.version,
            sourcePackage.dist,
            this.noDep,
            this._publish,
            dependencies.length,
            devDependencies.length, this.syncDevDependencies);

        if (dependencies.length > config.maxDependencies) {
            dependencies = dependencies.slice(0, config.maxDependencies);
        }

        if (devDependencies.length > config.maxDependencies) {
            devDependencies = devDependencies.slice(0, config.maxDependencies);
        }

        if (!this.noDep) {
            for (var i = 0; i < dependencies.length; i++) {
                this.add(dependencies[i]);
            }

            for (var i = 0; i < devDependencies.length; i++) {
                this.add(devDependencies[i]);
            }
        }

        // add module dependence
        await packageService.addDependencies(sourcePackage.name, dependencies);

        var shasum: any = crypto.createHash('sha1');
        var dataSize = 0;

        const afterUpload = async (result, remoteAbbreviatedVersion) => {
            //make sure sync module have the correct author info
            //only if can not get maintainers, use the username
            var author = username;
            if (Array.isArray(sourcePackage.maintainers) && sourcePackage.maintainers.length > 0) {
                author = sourcePackage.maintainers[0].name || username;
            } else if (sourcePackage._npmUser && sourcePackage._npmUser.name) {
                // try to use _npmUser instead
                author = sourcePackage._npmUser.name;
                sourcePackage.maintainers = [sourcePackage._npmUser];
            }

            var mod = {
                version: sourcePackage.version,
                name: sourcePackage.name,
                package: sourcePackage,
                author: author,
                publish_time: sourcePackage.publish_time,
            };

            // delete _publish_on_cnpm, because other cnpm maybe sync from current cnpm
            delete mod.package._publish_on_cnpm;
            if (this._publish) {
                // sync as publish
                mod.package._publish_on_cnpm = true;
            }

            // keep orgin dist fields, like `integrity` and so on
            var dist = Object.assign({}, sourcePackage.dist, {
                tarball: '',
                shasum: shasum,
                size: dataSize,
                noattachment: dataSize === 0,
            });

            if (result.url) {
                dist.tarball = result.url;
            } else if (result.key) {
                dist.key = result.key;
                dist.tarball = result.key;
            }

            mod.package.dist = dist;
            var r = await packageService.saveModule(mod);
            var moduleAbbreviatedId = null;
            if (config.enableAbbreviatedMetadata) {
                var moduleAbbreviatedResult = await packageService.saveModuleAbbreviated(mod, remoteAbbreviatedVersion);
                moduleAbbreviatedId = moduleAbbreviatedResult.id;
            }
            // @ts-ignore
            this.log('    [%s:%s] done, insertId: %s, author: %s, version: %s, '
                + 'size: %d, publish_time: %j, publish on cnpm: %s, '
                + 'moduleAbbreviatedId: %s',
                sourcePackage.name, versionIndex,
                r.id,
                author, mod.version, dataSize,
                new Date(mod.publish_time),
                this._publish,
                moduleAbbreviatedId);

            return r;
        }

        try {
            // get tarball
            // logger.syncInfo('[sync_module_worker] downloading %j to %j', downurl, filepath);
            var r;
            try {
                r = await urllib.request(downurl, downloadOptions);
            } catch (err) {
                // logger.syncInfo('[sync_module_worker] download %j to %j error: %s', downurl, filepath, err);
                throw err;
            }

            var statusCode = r.status || -1;
            // https://github.com/cnpm/cnpmjs.org/issues/325
            // if (statusCode === 404) {
            //   shasum = sourcePackage.dist.shasum;
            //   return yield afterUpload({
            //     url: downurl
            //   });
            // }

            if (statusCode !== 200) {
                var err = new Error('Download ' + downurl + ' fail, status: ' + statusCode);
                err.name = 'DownloadTarballError';
                // @ts-ignore
                err.data = sourcePackage;
                // @ts-ignore
                err.status = statusCode;
                // @ts-ignore
                // logger.syncInfo('[sync_module_worker] %s', err.message);
                throw err;
            }

            // read and check
            var rs = fs.createReadStream(filepath);
            rs.on('data', function (data) {
                shasum.update(data);
                dataSize += data.length;
            });
            var end = thunkify.event(rs);
            await end(); // after end event emit

            if (dataSize === 0) {
                var err = new Error('Download ' + downurl + ' file size is zero');
                err.name = 'DownloadTarballSizeZeroError';
                // @ts-ignore
                err.data = sourcePackage;
                throw err;
            }

            // check shasum
            shasum = shasum.digest('hex');
            if (shasum !== sourcePackage.dist.shasum) {
                var err = new Error('Download ' + downurl + ' shasum:' + shasum +
                    ' not match ' + sourcePackage.dist.shasum);
                err.name = 'DownloadTarballShasumError';
                // @ts-ignore
                err.data = sourcePackage;
                // logger.syncInfo('[sync_module_worker] %s', err.message);
                throw err;
            }

            var uploadOptions = {
                key: getCDNKey(sourcePackage.name, filename),
                size: dataSize,
                shasum: shasum
            };
            // upload to NFS
            // logger.syncInfo('[sync_module_worker] uploading %j to nfs', uploadOptions);
            var result;
            try {
                result = await config.nfs.upload(filepath, uploadOptions);
            } catch (err) {
                // logger.syncInfo('[sync_module_worker] upload %j to nfs error: %s', err);
                throw err;
            }
            // logger.syncInfo('[sync_module_worker] uploaded, saving %j to database', result);
            var r: any = await afterUpload(result, remoteAbbreviatedVersion);
            // logger.syncInfo('[sync_module_worker] sync %s@%s done!',
            //     sourcePackage.name, sourcePackage.version);
            return r;
        } finally {
            // remove tmp file whatever
            fs.unlink(filepath, utility.noop);
        }


    }
    add(name: string) {
        if (this.nameMap[name]) {
            return;
        }
        this.nameMap[name] = true;
        this.names.push(name);
        this.emit('add', name);
        // @ts-ignore
        this.log('    add dependencies: %s', name);
    }
    async _savePackageJsonBackup(pkgName: string, version: string) {
        const cdnKey = getPackageFileCDNKey(pkgName, version);
        const filePath = getTarballFilepath(pkgName, version, `package-${version}.json`);
        // @ts-ignore
        this.log('[%s:%s] start backup package.json', pkgName, version);
        // If package.json exists no need to sync
        try {
            await config.nfs.download(cdnKey, filePath);
            fs.unlink(filePath, utility.noop);
            // @ts-ignore
            this.log('[%s:%s] package.json exits skip backup', pkgName, version);
            // Download success, no need to sync
            return;
        } catch (_) {
            // ...
        }
        // Version is from db, so mod can not be null
        const mod = await packageService.showPackage(pkgName, version, {
            protocol: config.backupProtocol,
        });

        const file = JSON.stringify(mod.package);
        await mzFs.writeFile(filePath, file);

        let shasum: any = crypto.createHash('sha1');
        shasum.update(file);
        shasum = shasum.digest('hex');

        await config.nfs.upload(filePath, {
            key: cdnKey,
            size: file.length,
            shasum: shasum,
        });
        // @ts-ignore
        this.log('[%s:%s] package.json backup success', pkgName, version);
    }

    static sync = async (name, username, options?)=> {
        options = options || {};
        var result = await logService.create({ name: name, username: username });
        var worker = new SyncModuleWorker({
            logId: result.id,
            type: options.type,
            name: name,
            username: username,
            noDep: options.noDep,
            publish: options.publish,
            syncUpstreamFirst: options.syncUpstreamFirst,
            syncFromBackupFile: options.syncFromBackupFile,
            syncPrivatePackage: options.syncPrivatePackage
        });
        worker.start();
        return result.id;
    }
    

}

export = SyncModuleWorker;